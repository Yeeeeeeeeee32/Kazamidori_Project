...
def simulate_once(elev: float, azi: float, params: dict[str, Any], trial_idx: int = 0) -> dict:
    ...
    try:
        ...
        # ⚡ Bolt Optimization: Hoist invariant calculations out of the inner closures.
        # These closures are called thousands of times by the LSODA solver; calculating
        # temperature to Kelvin and lapse rates here prevents redundant math operations.
        t0_k = t0 + 273.15
        lapse_over_t0 = 0.0065 / t0_k

        def _calc_temp(h):
            return t0_k - 0.0065 * h

        def _calc_pres(h):
            return p0 * (1.0 - lapse_over_t0 * h) ** 5.2561
