    ...
    def _build(self) -> None:
        ...
        def _dsb(hi, dec, step, suffix):
            ...
            le.editingFinished.connect(
                lambda w=le: w.setStyleSheet("" if w.text().strip() != "" and w.hasAcceptableInput() else "border: 1px solid red;")
            )
            return le
...
class AppWindow(QMainWindow):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.af_backfire_input.editingFinished.connect(
            lambda w=self.af_backfire_input: w.setStyleSheet("" if w.text().strip() != "" and w.hasAcceptableInput() else "border: 1px solid red;")
        )
    ...
    def _build_airframe_page(self) -> QScrollArea:
        ...
        self.motor_cg_input.editingFinished.connect(
            lambda w=self.motor_cg_input: w.setStyleSheet("" if w.text().strip() != "" and w.hasAcceptableInput() else "border: 1px solid red;")
        )
        ...
        self.motor_dry_mass_input.editingFinished.connect(
            lambda w=self.motor_dry_mass_input: w.setStyleSheet("" if w.text().strip() != "" and w.hasAcceptableInput() else "border: 1px solid red;")
        )
    ...
    def _draw_real_result(self, ax, res: dict) -> None:
        ...
        _spd  = s.wind_profile[0]["speed_ms"] if s.wind_profile else 0.0
        _dir  = s.wind_profile[0]["dir_deg"] if s.wind_profile else 0.0
    ...
    def update_wind_plot(self) -> None:
        ...
        if not nodes:
            nodes = list(getattr(self.state, "wind_profile", []))
            if not nodes:
                nodes = [{"alt_m": 0.0, "speed_ms": 0.0, "dir_deg": 0.0}]
