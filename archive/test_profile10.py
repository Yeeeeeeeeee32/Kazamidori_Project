...
def _perturb_wind_profile_opt4(
    ...
    for i in range(n):
        ...
        if has_gust:
            ...
            # Since random variables X and Y are independent normally distributed variables
            # centered around 0, their sum X + Y is normally distributed with mean 0
            # and variance Var(X) + Var(Y).
            # This allows us to use one rng.gauss call instead of two!
...
def run():
    for _ in range(10000):
        _perturb_wind_profile_opt4(base_u, base_v, rng, 0.2, 1.0)
