...
def _perturb_wind_profile_opt8(
    ...
    # Extract all values to lists to speed up inner loop access
    # but that might be slower because of python loops
    # Let's try combining the two loops for has_gust and not has_gust
    # to avoid the branching inside the loop, which we already did.
    ...
    if has_gust:
        ...
        for i, ((alt_u, u_nom), (_, v_nom)) in enumerate(zip(u_prof, v_prof)):
            ...
            # 2. Local (mesoscale) turbulence
            local_spd = math_hypot(u_nom, v_nom)
            sigma     = wu_030 * local_spd if local_spd > 1.0 else wu_030
...
def run():
    for _ in range(10000):
        _perturb_wind_profile_opt8(base_u, base_v, rng, 0.2, 1.0)
